import React, { useState, useEffect, useContext, Fragment, forwardRef } from "react"
...
export default function Proponents() {
  ...
  const refreshTableData = () => {
    axios.get(`${pageContext.webAbsoluteUrl}/_api/web/lists/getByTitle('Proponents')/items`)
      .then(proponentResponse => {
        axios.get(`${pageContext.webAbsoluteUrl}/_api/web/RoleAssignments?$expand=Member,Member/Users`)
          .then(roleAssignmentsResponse => {
            ...
            setTable({ ...table, data: proponentResponse.data.value })
          })
      })
  }
  ...
  return (
    <Fragment>
      ...
      <DisableProponent open={proponentDisableDialog} close={proponentDisableHandleClose} proponent={currentProponent} proponentName={currentProponentName} />
      <Users {...users} />
      <ProponentLibrary open={proponentLibraryDialog} close={proponentLibraryHandleClose} proponent={currentProponent} />
      <ProponentQuestions open={proponentQuestionDialog} close={proponentQuestionHandleClose} proponent={currentProponent} />
    </Fragment>
  )
}